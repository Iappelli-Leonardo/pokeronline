...
public class CustomUtenteRepositoryImpl implements CustomUtenteRepository{
	...
	public List<Utente> findByExample(Utente example, String[] ruoliExample) {
		...
		StringBuilder queryBuilder = new StringBuilder("select u from Utente u join u.ruoli r where u.id = u.id ");
		...
		String role = "";
		if (ruoliExample != null && ruoliExample.length > 0) {
			for (int i = 0; i < ruoliExample.length; i++) {
				if (i == 0)
					role += " r.id = " + ruoliExample[i];
				else
					role += " or r.id = " + ruoliExample[i];
			}
		}
